					...
					<tbody>
						...
						<tr>
							<td>
								...
								<Form.Item name="userName" label="Имя пользователя" style={{ marginBottom: "0px" }} rules={[{ required: true, min: 1, type: "string", message: errMessage }]}>
									<div><Input value={store.data.userName} onChange={changeUserName} /></div>
								</Form.Item>

								<Form.Item name="password" label="Пароль" style={{ marginBottom: "0px" }} rules={[{ required: true, min: 1, type: "string", message: errMessage }]}>
						...
						<tr>
							<td align="right">
								<Button icon={<SaveOutlined />} onClick={(event) => submit(event)}
									style={{ background: theme.background, color: theme.color }}>Войти</Button>
							</td>
						</tr>
					</tbody>
				</table>
