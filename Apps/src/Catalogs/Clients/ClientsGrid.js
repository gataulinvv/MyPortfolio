...
const ClientsGrid = observer((({ title }) => {
	...
	const columns = [
		...
		{
			//width: '20%',
			title: () => <StringFilter columnName='name' columnTitle='Имя' dataSource={store.data} parentViewStore={store} />,
			dataIndex: 'name',
			key: 'name',
			sorter: {
		...
		},
		{
			...
			render: (text, record, index) => {
				return (
					<BasePopper titleType={"edit"} header="Карточка клиента" showcase={record.name} Content={<ClientForm id={record.id} parentViewStore={store} />} />
				)
			}
		},
			...
			<table width="100%">
				<tbody>
					...
					<tr><td><Button icon={<RedoOutlined />} onClick={() => store.RefreshStore()}
						...
						<td align='right'>
							<BasePopper titleType={"add"} header="Карточка клиента" showcase={"Новый клиент"} Content={<ClientForm id={0} parentViewStore={store} />} />
						</td>
					</tr>
					</tbody>
				</table>
